...
exec(`npx hardhat starknet-estimate-fee --starknet-network ${NETWORK} --contract contract --function sum_points_to_tuple --address ${ADDRESS} 2>&1 \
    | tail -n +6 \
    | diff - ${path.join(PREFIX, "no-inputs.txt")}`);
console.log("Success");

console.log("Testing too few input arguments");
exec(`npx hardhat starknet-estimate-fee --starknet-network ${NETWORK} --contract contract --function sum_points_to_tuple --address ${ADDRESS} --inputs "10 20 30" 2>&1 \
    | tail -n +6 \
    | diff - ${path.join(PREFIX, "too-few-inputs.txt")}`);
console.log("Success");

console.log("Testing too many input arguments");
exec(`npx hardhat starknet-estimate-fee --starknet-network ${NETWORK} --contract contract --function sum_points_to_tuple --address ${ADDRESS} --inputs "10 20 30 40 50" 2>&1 \
    | tail -n +6 \
...
// exec(`npx hardhat starknet-estimate-fee --starknet-network ${NETWORK} --contract contract --function sum_points_to_tuple --address ${ADDRESS} --inputs "10 20 30 40" 2>&1 \
//     | tail -n +2 \
//     | head -n -3 \
//     | diff - <(echo "40 60")`);
// console.log("Success");
